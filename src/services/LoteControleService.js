// Service generated from LoteControleService.java
const db = require('../models');
const NotFoundError = require('../utils/NotFoundError');

const Model = db.LoteControleModel;

const LoteControleDto = require('../dtos/LoteControleDto');

class LoteControleService {
  // model -> dto
  modelParaDto(model) {
    if (!model) return null;
    // Sequelize instance: use dataValues
    const data = model.dataValues ? model.dataValues : model;
    return new LoteControleDto(data).toPlain();
  }
...
module.exports = new LoteControleService();
